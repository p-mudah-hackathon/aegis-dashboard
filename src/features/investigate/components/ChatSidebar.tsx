...
export const ChatSidebar: React.FC<ChatSidebarProps> = ({
	...
	return (
		<aside
			className='bg-[#0f0f0f] border-r border-white/5 flex flex-col shrink-0 transition-[width] duration-300 ease-in-out'
			style={{ width: collapsed ? 56 : 288 }}
		>
			<div className='shrink-0 p-2'>
				<button
					onClick={onToggle}
					className='size-10 rounded-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors'
			...
			<div
				className='shrink-0 border-b border-white/5 transition-all duration-300'
				style={{ padding: collapsed ? '0 6px 12px' : '0 8px 12px' }}
			>
				<button
					onClick={onNewChat}
					className='flex items-center justify-center border border-orange-500/30 text-orange-400 transition-all duration-300 w-full'
					style={{
						height: 40,
			...
			>
				<div className='h-full overflow-y-auto px-2 pb-2 custom-scrollbar'>
					<p className='text-[10px] text-gray-600 font-semibold uppercase tracking-wider px-3 py-2'>
						Recent
					</p>
					{sessions.length === 0 ? (
							...
							{sessions.map((s) => {
								...
								return (
									...
									>
										<MessageSquare
											size={14}
											className={`mt-0.5 shrink-0 ${isActive ? 'text-orange-400' : 'text-gray-500'}`}
										/>
