...
export const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
	...
	return (
		...
		>
			...
			<div
				className='relative bg-[#121212] border border-white/10 rounded-2xl w-full max-w-[800px] max-h-[85vh] shadow-[0_30px_80px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden'
			...
			>
				...
				>
					...
					<div className='flex items-center gap-4 p-4 bg-white/[0.02] rounded-xl border border-white/5'>
						<div className={`text-3xl font-bold ${getRiskColor(txn.risk_score)}`}>
							{(txn.risk_score * 100).toFixed(1)}%
						</div>
						<div>
							<p className='text-xs text-gray-400'>HTGNN Risk Score</p>
							<p className={`text-sm font-bold ${getRiskColor(txn.risk_score)}`}>
								{txn.risk_score >= 0.7 ? 'HIGH RISK' : txn.risk_score >= 0.4 ? 'MEDIUM RISK' : 'LOW RISK'}
							</p>
						</div>
						{txn.fraud_type && (
							<div className='ml-auto px-3 py-1.5 bg-purple-500/10 rounded-lg border border-purple-500/20'>
								<p className='text-[10px] text-purple-300 uppercase tracking-wider'>Fraud Type</p>
								<p className='text-sm font-bold text-purple-400'>
									{txn.fraud_type.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
								</p>
							</div>
						)}
					</div>
					...
					<div className='grid grid-cols-2 gap-4'>
						...
						<div className='bg-white/[0.02] rounded-xl p-4 border border-white/5'>
							...
							<div className='space-y-0'>
								...
								<InfoRow label='Foreign Amount' value={txn.amount_foreign.toFixed(2)} />
							</div>
						</div>
						...
						<div className='bg-white/[0.02] rounded-xl p-4 border border-white/5'>
							...
							{txn.xai_reasons && txn.xai_reasons.length > 0 ? (
								<div className='space-y-2'>
									{txn.xai_reasons.sort((a, b) => b.importance - a.importance).map((f, i) => (
								...
								<p className='text-gray-600 text-xs italic'>No XAI features available</p>
							)}
						</div>
					</div>
					...
					<div className='flex gap-3 pt-2 pb-2'>
						<button
							...
							className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[12px] font-bold border transition-all ${txn.review_status === 'confirmed_fraud'
						...
						<button
							...
							className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[12px] font-bold border transition-all ${txn.review_status === 'false_positive'
