...
export const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
	...
	return (
		...
		>
			...
			<div
				className='relative bg-[#121212] border border-white/10 rounded-2xl w-full max-w-[1000px] max-h-[85vh] shadow-[0_30px_80px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden'
			...
			>
				...
				<div className='flex flex-1 min-h-0'>
					{/* Left: Transaction Info (scrollable, hidden scrollbar) */}
					<div
						className='w-[400px] shrink-0 border-r border-white/5 overflow-y-auto px-6 py-4 space-y-4'
						style={{
							scrollbarWidth: 'none',
							msOverflowStyle: 'none',
						}}
					>
						<style>{`.modal-left::-webkit-scrollbar { display: none; }`}</style>
						...
						<div className='bg-white/[0.02] rounded-xl p-4 border border-white/5'>
							...
							<div className='space-y-0'>
								...
								<div className='flex justify-between items-center py-1.5'>
									...
									<span
										className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold border ${getRiskBadge(txn.merchantRiskScore)}`}
									>
					...
					{/* Right: Fund Flow Graph (fills remaining space) */}
